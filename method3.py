# -*- coding: utf-8 -*-
"""[DScover_Main](변수 수정)0615_2_1DCNN_편향적음_0.589.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1FJGXWGn_V3T3hSH00rY1LOD7d1J96NW2
...
# 데이터 폴더 경로
folder_path = '/content/drive/MyDrive/[DScover]Main/ch2025_data_items' #여긴 알아서 수정

parquet_files = [f for f in os.listdir(folder_path) if f.endswith('.parquet')]

# 파일별로 DataFrame 저장 (딕셔너리 형태)
dataframes = {}

for file in parquet_files:
    file_path = os.path.join(folder_path, file)
    df = pd.read_parquet(file_path)
    key_name = file.replace('.parquet', '')  # 확장자 제외
    dataframes[key_name] = df
    print(f"{key_name}: {df.shape}")  # 각 DataFrame의 크기 출력

for key in dataframes:
    globals()['df_' + key.replace('ch2025_', '')] = pd.DataFrame(dataframes[key])

#m_gps 컬럼의 딕셔너리 데이터를 개별 컬럼으로 펼치는 함수

import ast
import numpy as np

def expand_gps_columns(df):
    ...
    gps_data = df['m_gps'].apply(extract_first_dict)

    gps_df = pd.DataFrame(gps_data.tolist())

    gps_df.columns = [f'm_{col}' for col in gps_df.columns]

    result_df = pd.concat([df.drop('m_gps', axis=1), gps_df], axis=1)

    return result_df

df_mGPS_f = expand_gps_columns(df_mGps)

train_df = pd.read_csv('/content/drive/MyDrive/[DScover]Main/ch2025_metrics_train.csv')
test_df = pd.read_csv('/content/drive/MyDrive/[DScover]Main/ch2025_submission_sample.csv')
...
# 1. 공통 전처리 ─ timestamp → datetime  &  1분 단위 정규화
sensor_dfs = [df_mActivity, df_mGPS_f, df_mScreenStatus, df_wHr, df_wLight]

for df in sensor_dfs:
    df['timestamp'] = pd.to_datetime(df['timestamp']).dt.floor('T')   # ① 형 변환 ② 1-minute floor


# 3. subject_id·timestamp로 단계적 outer-merge
df_merged = reduce(
...
# 결과: subject_id, timestamp, m_activity, m_altitude, m_latitude, m_longitude,
#       m_speed, m_screen_use, heart_rate, w_light  ---- (NaN 허용)

import numpy as np

df_merged["heart_rate"] = df_merged["heart_rate"].apply(lambda x: np.mean(x) if isinstance(x, (list, np.ndarray)) else x)

df_merged = df_merged.drop(["m_altitude", "m_latitude", "m_longitude"], axis=1)
...
class AdvancedCNN(nn.Module):
    ...
    def __init__(self, input_channels=5, sequence_length=1440, num_outputs=6):
...
def main():
    ...
    print("\nPredictions saved to 'enhanced_5feature_cnn_predictions.csv'")
